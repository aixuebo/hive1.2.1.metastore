...
public class ObjectStore implements RawStore, Configurable {
  private static Properties prop = null;//数据库的配置文件信息
  ...
  */
  ...
  private boolean isInitialized = false;//true表示已经初始化完成
  private PersistenceManager pm = null;
  private MetaStoreDirectSql directSql = null;
  private PartitionExpressionProxy expressionProxy = null;
  private Configuration hiveConf;
  int openTrasactionCalls = 0;//打开事物数量,一个事物内部可能会打开事物,因此该值是可能会>1的
  ...
  public void setConf(Configuration conf) {
    ...
    try {
      isInitialized = false;
      hiveConf = conf;
      Properties propsFromConf = getDataSourceProps(conf);//读取连接mysql的配置文件信息
      boolean propsChanged = !propsFromConf.equals(prop);//如果属性跟以前配置的不相同,则返回true,说明配置文件被更改了
      ...
      initialize(propsFromConf);//根据配置文件,进行初始化
   ...
   * in jpox.properties.
   * 读取配置文件中datanucleus和jdo中的key-value作为数据库的配置文件
   */
  @SuppressWarnings("nls")
  ...
  public boolean openTransaction() {
    ...
    if (openTrasactionCalls == 1) {
      ...
      // openTransactionCalls > 1 means this is an interior transaction 如果当前事物数量>1,说明这个是内部的事物
      // We should already have a transaction created that is active.我们应该已经有一个事物被创建了
      if ((currentTransaction == null) || (!currentTransaction.isActive())){//当前必须有事物,并且事物必须或者
  ...
  public boolean commitTransaction() {
    if (TXN_STATUS.ROLLBACK == transactionStatus) {//说明当前事物是回滚的,则不需要提交
      debugLog("Commit transaction: rollback");
      return false;
    }
    if (openTrasactionCalls <= 0) {//如果事物数量<0,说明有异常,不能提交
    ...
    if (!currentTransaction.isActive()) {//事物不存活
      RuntimeException e = new RuntimeException("commitTransaction was called but openTransactionCalls = "
          + openTrasactionCalls + ". This probably indicates that there are unbalanced " +
          "calls to openTransaction/commitTransaction");
      LOG.error(e);
    ...
    openTrasactionCalls--;//减少一个事物,因为事物提交一个了
    debugLog("Commit transaction: count = " + openTrasactionCalls + ", isactive "+ currentTransaction.isActive());

    if ((openTrasactionCalls == 0) && currentTransaction.isActive()) {//只有当事物数量=0,说明内部事物都提交完了,才会真正的变成提交状态,并且提交事物
  ...
  public void rollbackTransaction() {
    if (openTrasactionCalls < 1) {//回滚的话必须有事物,因此一定是>1的
    ...
    try {
      if (currentTransaction.isActive()
          && transactionStatus != TXN_STATUS.ROLLBACK) {//必须是活跃的事物,并且此时不是回滚状态
        currentTransaction.rollback();//执行回滚
      }
    } finally {//回滚后,设置初始化数据
      openTrasactionCalls = 0;
  ...
  //插入数据的时候,对象与mysql的属性对应关系在项目的根目录下的package.do下
  @Override
  public void createDatabase(Database db) throws InvalidObjectException, MetaException {
    ...
    PrincipalType ownerType = db.getOwnerType();//user role group三者之一
  ...
  private MDatabase getMDatabase(String name) throws NoSuchObjectException {
    ...
    try {
      openTransaction();
      name = HiveStringUtils.normalizeIdentifier(name);//格式化name字符串
      Query query = pm.newQuery(MDatabase.class, "name == dbname");//根据name=一个变量
      query.declareParameters("java.lang.String dbname");//变量dname是String类型的
      query.setUnique(true);
      mdb = (MDatabase) query.execute(name);//设置dname变量为name值
      pm.retrieve(mdb);
      commited = commitTransaction();
    } finally {
      if (!commited) {
   ...
   * 重新设置db的paramter和ownerName、ownerType
   */
  @Override
  public boolean alterDatabase(String dbName, Database db)
    ...
    try {
      mdb = getMDatabase(dbName);//查询数据库对象
      mdb.setParameters(db.getParameters());
      mdb.setOwnerName(db.getOwnerName());
  ...
  public boolean dropDatabase(String dbname) throws NoSuchObjectException, MetaException {
    ...
    try {
      ...
      MDatabase db = getMDatabase(dbname);//查询数据库
      pm.retrieve(db);
      if (db != null) {
    ...
    /**
     * 通配符查询数据库集合
     * @param pattern 比如.*查询所有数据库 或者 show databases like 'jl*|od*';
     */
  @Override
...
//
  ...
  private abstract class GetHelper<T> {
    private final boolean isInTxn, doTrace, allowJdo; //isInTxn true 表示当前正在在事物中 doTrace true 表示log4J打印debug级别日志,allowJdo 表示可以使用jdo规范
    private boolean doUseDirectSql;
    private long start;
    private Table table;
    protected final String dbName, tblName;//数据库名字 和 表名字
